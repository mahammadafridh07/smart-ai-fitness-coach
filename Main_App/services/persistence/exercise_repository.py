...
def init_db() -> None:
    ...
    _migrate_users_table_for_auth(conn)


def _migrate_users_table_for_auth(conn: sqlite3.Connection) -> None:
    """
    Add email/password_hash columns to a pre-existing `users` table
    (from the old username-only login wall) without losing any data.
    """
    existing_columns = {
        row["name"]
...
def get_user_by_email(email: str) -> sqlite3.Row:
    conn = _get_connection()
...
def get_user_by_username_or_email(identifier: str) -> sqlite3.Row:
    conn = _get_connection()
